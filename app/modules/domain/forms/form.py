...
class EditDomain(Form):
    domain_name = TextField('Domain Name', [validators.Required('Please enter the domain name that you wish to add')])
    vhost = TextAreaField('Domain\'s Virtual Host Configuration')#default=Domain(domain='testing.com',service='httpd').getDomainVirtualHost())
    phpini = TextAreaField('Domain\'s PHP Settings')#default=Domain(domain='testing.com',service='phpini').getDomainPhpini())
